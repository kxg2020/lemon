<template>
  <div class="wh100">
    <div class="appbar" :class="navHidden ? 'nav-hidden-appbar' : ''">
      <div class="left">
        <button @click="changeNavStatus" class="icon-button" style="height: 64px"><i class="ion-navicon-round"></i>
        </button>
      </div>
      <div class="center">
        <span>{{pathName}}</span>
    ...
    <div class="paper" :class="navHidden ? '' : 'open-paper'">
      ...
      <div class="paper-menu">
        <el-scrollbar style="position: absolute;top: 64px;bottom: 0;width: 256px;">
          <el-menu :default-active="defaultActive" class="el-menu-vertical-demo" theme="dark" style="border-radius: 0;"
                   :router="routerState" :unique-opened="true">
            <template v-for="(item, index) in $router.options.routes" v-if="item.children">
              ...
              <template v-else>
                <el-menu-item :index="item.children[0].path"><i :class="item.children[0].icon"
                                                                style="padding-right: 10px"></i>{{item.children[0].name}}
                </el-menu-item>
              </template>
            </template>
          </el-menu>
...
<script type="text/ecmascript-6">
  export default {
    ...
    created: function () {
      ...
      this.defaultActive = that.$route.path
      this.pathName = that.$route.name
    },
    methods: {
      logout: function () {
        var _this = this
